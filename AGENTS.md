...
- **Auth**: per-mailbox password (WebCrypto PBKDF2, 100k iters, salted). Login issues a 32-byte session token stored in `auth_sessions` (7-day expiry). All mailbox-level API routes require `Authorization: Bearer <token>`; token must match the `:id` path param. Prefer `POST /api/auth/login` (localPart+password); `POST /api/auth/logout` revokes session (`?all=1` revokes all). Session management: `GET /api/mailboxes/:id/sessions` lists active sessions (with `isCurrent`), `DELETE /api/mailboxes/:id/sessions/:sessionId` revokes another session (current → 400 `cannot_revoke_current`). No public mailbox listing.
